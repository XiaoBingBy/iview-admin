    ...
    <Card>
      <Row>
        <Form ref="formInline" inline :label-width="70">
          <Form-item label="角色名">
            <Input v-model="filter.querys[0].value" type="text" clearable placeholder="请输入角色名"/>
          </Form-item>
      ...
      <Row class="operation" type="flex" justify="space-between">
        <div class="operation-group">
          ...
          <Button icon="md-trash" @click="handleDeletRole">删除</Button>
        </div>
    ...
    <Modal ref="addOrUpdateModal" v-model="view.showAddOrUpdateModal" :title="view.addOrUpdateTitle" :mask-closable="false" ok-text="提交" :loading="true" @on-ok="onOk" :width="600">
      <Form ref="addOrUpdateFormRef" :model="addOrUpdateRoleDto" :rules="roleRuleValidate" :label-width="80">
        ...
        <FormItem prop="roleCode" label="角色编码">
          <Input v-model="addOrUpdateRoleDto.roleCode"></Input>
        </FormItem>
        <FormItem label="排序号">
...
<script>
import { apiGetRoleList, apiAddRole, apiUpdateRole, apiDeleteRole, apiGetRolePermissionTree, apiUpdateRolePermission } from '@/api/upms/role'
...
export default {
  ...
  methods: {
    ...
    handleUpdateRole (row) {
      ...
      let { id, name, roleCode, sort, status, description } = row
      this.addOrUpdateRoleDto = { id, name, roleCode, sort, status, description }
      this.$refs.addOrUpdateFormRef.resetFields()
      this.view.showAddOrUpdateModal = true
    },
    onOk () {
