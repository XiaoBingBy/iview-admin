  ...
  <Card>
    <Row>
      <Form ref="formInline" inline :label-width="70">
        <Form-item label="字典名:">
          <Input type="text" v-model="filter.querys[0].value" clearable placeholder="请输入字典名"/>
        </Form-item>
    ...
    <Row class="operation" type="flex" justify="space-between">
      <div class="opt-group">
        ...
        <Button class="opt-btn" icon="md-trash" @click="handleDeletDict">删除</Button>
      </div>
      <div>
    ...
    <Modal ref="addOrUpdateModal" v-model="view.showAddOrUpdateModal" :title="view.addOrUpdateTitle" :mask-closable="false" ok-text="提交" :loading="true" @on-ok="onOk" :width="600">
      <Form ref="addOrUpdateFormRef" :model="addOrUpdateDiscDto" :rules="discRuleValidate" :label-width="80">
        <FormItem prop="name" label="字典名">
          <Input v-model="addOrUpdateDiscDto.name"></Input>
        </FormItem>
        <FormItem prop="type" label="类型">
          <Input v-model="addOrUpdateDiscDto.type"></Input>
        </FormItem>
        <FormItem prop="code" label="编码">
          <Input v-model="addOrUpdateDiscDto.code"></Input>
        </FormItem>
        <FormItem prop="value" label="值">
          <Input v-model="addOrUpdateDiscDto.value"></Input>
        </FormItem>
        <FormItem label="排序号">
          <InputNumber v-model="addOrUpdateDiscDto.sort"></InputNumber>
        </FormItem>
        <FormItem label="状态">
          <i-switch size="large" v-model="addOrUpdateDiscDto.status" :true-value="1" :false-value="0">
            <span slot="open">正常</span>
            <span slot="close">禁用</span>
          </i-switch>
        </FormItem>
        <FormItem prop="description" label="备注">
          <Input type="textarea" v-model="addOrUpdateDiscDto.description"/>
        </FormItem>
      </Form>
    </Modal>
...
<script>
import { apiGetDiscList, apiAddDisc, apiUpdateDisc, apiDeleteDict } from '@/api/upms/dict'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      apiGetDiscList(this.filter).then(res => {
    ...
    handleAddRole: function () {
      this.view.addOrUpdateTitle = '添加字典'
      this.view.isAddOrUpdate = true
      this.addOrUpdateDiscDto = {
        name: '',
        type: '',
        code: '',
        value: '',
    ...
    handleUpdateRole: function (row) {
      this.view.addOrUpdateTitle = '修改字典'
      ...
      this.$refs.addOrUpdateFormRef.validate(valid => {
        if (valid) {
          if (this.view.isAddOrUpdate) {
            apiAddDisc(this.addOrUpdateDiscDto).then(res => {
              this.$Message.success('添加成功')
              this.view.showAddOrUpdateModal = false
              this.getList()
            })
          } else {
            apiUpdateDisc(this.addOrUpdateDiscDto).then(res => {
    ...
    deletDict: function (row) {
      this.$Modal.confirm({
        title: '确认删除',
        content: '您确认要删除字典' + row.name + ' ?',
        onOk: () => {
          let ids = ''
          ids = row.id
          apiDeleteDict(ids).then(res => {
